...
router.post("/", function (request, response) {
  ...
  mysql.query('SELECT * FROM UserrInfo WHERE id = ?', [id], function(error, results, fields) { // DB에 같은 이름의 회원아이디가 있는지 확인
    ...
    if (results.length <= 0 && password == passwordcheck) {     // DB에 같은 이름의 회원아이디가 없고, 비밀번호가 올바르게 입력된 경우 
      mysql.query("INSERT INTO UserrInfo(id, password, passwordcheck, name, birth, gender, email, phone, age) VALUES(?,?,?,?,?,?,?,?,?)"
    [id, password, passwordcheck, body.name, body.birth, body.gender, body.email, body.phone, body.age], function (error, results){
